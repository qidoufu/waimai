d4 = function() {
    ...
    d4.mean = function(array, f) {
        ...
        if (arguments.length === 1) {
            while (++i < n)
                if (d4_number(a = array[i])) m += (a - m) / ++j;
        } else {
            while (++i < n)
                if (d4_number(a = f.call(array, array[i], i))) m += (a - m) / ++j;
        }
    ...
    d4.nest = function() {
        ...
        function map(array, depth) {
            ...
            var i = -1,
                n = array.length,
                key = keys[depth++],
                keyValue, object, valuesByKey = new d4_Map(),
                values, o = {};
            while (++i < n) {
    ...
    d4.xhr = function(url, mimeType, callback) {
        var xhr = {},
            dispatch = d4.dispatch("progress", "load", "error"),
            headers = {},
            response = d4_identity,
    ...
    d4.text = function() {
        return d4.xhr.apply(d4, arguments).response(d4_text);
    };

    function d4_text(request) {
        return request.responseText;
    }
    d4.json = function(url, callback) {
        return d4.xhr(url, "application/json", callback).response(d4_json);
    };
    ...
    d4.html = function(url, callback) {
        return d4.xhr(url, "text/html", callback).response(d4_html);
    };
    ...
    d4.xml = function() {
        return d4.xhr.apply(d4, arguments).response(d4_xml);
    };

    function d4_xml(request) {
    ...
    var d4_format_re = /(?:([^{])?([<>=^]))?([+\- ])?(#)?(0)?([0-9]+)?(,)?(\.[0-9]+)?([a-zA-Z%])?/;
    ...
    d4.interpolateString = function(a, b) {
        ...
        for (i = 0, n = q.length;
            (m = d4_interpolate_number.exec(a)) && i < n; ++i) {
            o = q[i];
            if (o.x == m[0]) {
                if (o.i) {
                    if (s[o.i + 1] == null) {
    ...
    d4.interpolateTransform = function(a, b) {
        ...
        if (ta[0] != tb[0] || ta[1] != tb[1]) {
            ...
            q.push({
                i: 1,
                x: d4.interpolateNumber(ta[0], tb[0])
            }, {
                i: 3,
                x: d4.interpolateNumber(ta[1], tb[1])
            });
        } else if (tb[0] || tb[1]) {
            s.push("translate(" + tb + ")");
        } else {
        ...
        if (ka[0] != kb[0] || ka[1] != kb[1]) {
            ...
            q.push({
                i: n - 4,
                x: d4.interpolateNumber(ka[0], kb[0])
            }, {
                i: n - 2,
                x: d4.interpolateNumber(ka[1], kb[1])
            });
    ...
    d4.interpolateHsl = function(a, b) {
        ...
        return function(t) {
            return d4_hsl_rgb(h0 + h1 * t, s0 + s1 * t, l0 + l1 * t) + "";
        };
    };
    d4.interpolateLab = function(a, b) {
        a = d4.lab(a);
        b = d4.lab(b);
        var al = a.l,
            aa = a.a,
    ...
    d4.interpolateHcl = function(a, b) {
        a = d4.hcl(a);
        b = d4.hcl(b);
        var ah = a.h,
            ac = a.c,
    ...
    function d4_interpolateByName(name) {
        return name == "transform" ? d4.interpolateTransform : d4.interpolate;
    }
    d4.interpolators = [d4.interpolateObject, function(a, b) {
        return b instanceof Array && d4.interpolateArray(a, b);
    }, function(a, b) {
        return (typeof a === "string" || typeof b === "string") && d4.interpolateString(a + "", b + "");
    }, function(a, b) {
        return (typeof b === "string" ? d4_rgb_names.has(b) || /^(#|rgb\(|hsl\()/.test(b) : b instanceof d4_Color) && d4.interpolateRgb(a, b);
    }, function(a, b) {
        return !isNaN(a = +a) && !isNaN(b = +b) && d4.interpolateNumber(a, b);
    }];
    ...
    function d4_rgb_lab(r, g, b) {
        ...
        g = d4_rgb_xyz(g);
        b = d4_rgb_xyz(b);
        var x = d4_xyz_lab((.4124564 * r + .3575761 * g + .1804375 * b) / d4_lab_X),
            y = d4_xyz_lab((.2126729 * r + .7151522 * g + .072175 * b) / d4_lab_Y),
            z = d4_xyz_lab((.0193339 * r + .119192 * g + .9503041 * b) / d4_lab_Z);
    ...
    function d4_lab_hcl(l, a, b) {
        return d4_hcl(Math.atan2(b, a) / π * 180, Math.sqrt(a * a + b * b), l);
    }
    ...
    var d4_selectionPrototype = [];
    d4.selection = function() {
        return d4_selectionRoot;
    };
    d4.selection.prototype = d4_selectionPrototype;
        ...
        if (arguments.length < 2) {
            if (typeof name === "string") {
                ...
                if (value = node.classList) {
                    ...
                    while (++i < n)
                        if (!d4_selection_classedRe(name[i]).test(value)) return false;
                }
                return true;
            }
    ...
    function d4_selection_classedName(name) {
        ...
        return function(node, value) {
            ...
            if (value) {
                re.lastIndex = 0;
                if (!re.test(cv)) {
                    cv = d4_collapse(cv + " " + name);
                    if (cb) c.baseVal = cv;
                    else node.className = cv;
                }
            } else if (cv) {
                cv = d4_collapse(cv.replace(re, " "));
                if (cb) c.baseVal = cv;
                else node.className = cv;
            }
        };
    ...
    d4_selectionPrototype.insert = function(name, before) {
        ...
        function insert() {
            return this.insertBefore(d4_document.createElementNS(this.namespaceURI, name), d4_select(before, this));
        }

        function insertNS() {
            return this.insertBefore(d4_document.createElementNS(name.space, name.local), d4_select(before, this));
        }
    ...
    d4_selectionPrototype.transition = function() {
        var id = d4_transitionInheritId || ++d4_transitionId,
            subgroups = [],
            subgroup, node, transition = Object.create(d4_transitionInherit);
    ...
    var d4_selectionRoot = d4_selection([
        [d4_document]
    ]);
    d4_selectionRoot[0].parentNode = d4_selectRoot;
    d4.select = function(selector) {
        return typeof selector === "string" ? d4_selectionRoot.select(selector) : d4_selection([
            [selector]
        ]);
    };
    d4.selectAll = function(selector) {
        return typeof selector === "string" ? d4_selectionRoot.selectAll(selector) : d4_selection([d4_array(selector)]);
    };
    ...
    var d4_transitionPrototype = [],
        d4_transitionId = 0,
        d4_transitionInheritId, d4_transitionInherit = {
    ...
    d4_transitionPrototype.attr = function(nameNS, value) {
        ...
        return d4_transition_tween(this, "attr." + nameNS, value, function(b) {
            function attrString() {
    ...
    d4_transitionPrototype.style = function(name, value, priority) {
        ...
        return d4_transition_tween(this, "style." + name, value, function(b) {
            function styleString() {
                var a = d4_window.getComputedStyle(this, null).getPropertyValue(name),
                    i;
                return a !== b && (i = interpolate(a, b), function(t) {
    ...
    var d4_timer_id = 0,
        d4_timer_byId = {},
        d4_timer_queue = null,
        d4_timer_interval, d4_timer_timeout;
    d4.timer = function(callback, delay, then) {
        ...
        var timer = d4_timer_byId[callback.id];
        if (timer && timer.callback === callback) {
            timer.then = then;
            timer.delay = delay;
        } else d4_timer_byId[callback.id = ++d4_timer_id] = d4_timer_queue = {
            callback: callback,
            then: then,
            delay: delay,
            next: d4_timer_queue
    ...
    function d4_timer_flush() {
        var t0 = null,
            t1 = d4_timer_queue,
            then = Infinity;
        while (t1) {
            if (t1.flush) {
                delete d4_timer_byId[t1.callback.id];
                t1 = t0 ? t0.next = t1.next : d4_timer_queue = t1.next;
            } else {
    ...
    function d4_scale_niceDefault() {
        return Math;
    }
    d4.scale.linear = function() {
        return d4_scale_linear([0, 1], [0, 1], d4.interpolate, false);
    };
    ...
    function d4_scale_linear(domain, range, interpolate, clamp) {
        ...
        scale.tickFormat = function(m) {
            return d4_scale_linearTickFormat(domain, m);
        };
        scale.nice = function() {
    ...
    function d4_scale_linearTickFormat(domain, m) {
        return d4.format(",." + Math.max(0, -Math.floor(Math.log(d4_scale_linearTickRange(domain, m)[2]) / Math.LN10 + .01)) + "f");
    }
    ...
    d4.scale.log = function() {
        return d4_scale_log(d4.scale.linear(), d4_scale_logp);
    };

    function d4_scale_log(linear, log) {
        var pow = log.pow;
        ...
        scale.domain = function(x) {
            ...
            log = x[0] < 0 ? d4_scale_logn : d4_scale_logp;
            pow = log.pow;
            linear.domain(x.map(log));
            return scale;
        };
        scale.nice = function() {
            linear.domain(d4_scale_nice(linear.domain(), d4_scale_niceDefault));
            return scale;
        };
        ...
        scale.copy = function() {
            return d4_scale_log(linear.copy(), log);
        };
    ...
    d4_scale_logp.pow = function(x) {
        return Math.pow(10, x);
    };
    d4_scale_logn.pow = function(x) {
        return -Math.pow(10, -x);
    };
    d4.scale.pow = function() {
        return d4_scale_pow(d4.scale.linear(), 1);
    };

    function d4_scale_pow(linear, exponent) {
        ...
        scale.ticks = function(m) {
            return d4_scale_linearTicks(scale.domain(), m);
        };
        scale.tickFormat = function(m) {
            return d4_scale_linearTickFormat(scale.domain(), m);
        };
        scale.nice = function() {
            return scale.domain(d4_scale_nice(scale.domain(), d4_scale_linearNice));
        };
        ...
        scale.copy = function() {
            return d4_scale_pow(linear.copy(), exponent);
        };
        return d4_scale_linearRebind(scale, linear);
    }
    ...
    var d4_category20 = ["#fcebc3", "#fcebc3", "#fcebc3", "#ffb35b", "#fcebc3", "#fcebc3", "#fcebc3", "#fcebc3", "#fcebc3", "#fcebc3", "#fcebc3", "#fcebc3", "#e3e3e3", "#e3e3e3", "#e3e3e3", "#e3e3e3", "#dbdddd", "#dbdddd", "#dbdddd", "#dbdddd"];
    ...
    d4.svg.arc = function() {
        ...
        arc.centroid = function() {
            var r = (innerRadius.apply(this, arguments) + outerRadius.apply(this, arguments)) / 2,
                a = (startAngle.apply(this, arguments) + endAngle.apply(this, arguments)) / 2 + d4_svg_arcOffset;
    ...
    function d4_svg_lineMonotoneTangents(points) {
        var tangents = [],
            d, a, b, s, m = d4_svg_lineFiniteDifferences(points),
            i = -1,
            j = points.length - 1;
        while (++i < j) {
    ...
    d4.behavior.drag = function() {
        ...
        function mousedown() {
            ...
            function dragend() {
                ...
                if (moved) {
                    d4_eventCancel();
                    if (d4.event.target === eventTarget) w.on("click.drag", click, true);
                }
    ...
    d4.behavior.zoom = function() {
        var translate = [0, 0],
            translate0, scale = 1,
            scale0, scaleExtent = d4_behavior_zoomInfinity,
        ...
        function mousedown() {
            ...
            function mouseup() {
                ...
                if (moved && d4.event.target === eventTarget) w.on("click.zoom", click, true);
            }
    ...
    var d4_behavior_zoomDelta, d4_behavior_zoomWheel = "onwheel" in document ? (d4_behavior_zoomDelta = function() {
        return -d4.event.deltaY * (d4.event.deltaMode ? 120 : 1);
    }, "wheel") : "onmousewheel" in document ? (d4_behavior_zoomDelta = function() {
    ...
    d4.layout.pie = function() {
        ...
        function pie(data) {
            ...
            var k = ((typeof endAngle === "function" ? endAngle.apply(this, arguments) : endAngle) - startAngle) / d4.sum(values);
    ...
    var d4_layout_stackOrders = d4.map({
        "inside-out": function(data) {
            var n = data.length,
                i, j, max = data.map(d4_layout_stackMaxIndex),
                sums = data.map(d4_layout_stackReduceSum),
                index = d4.range(n).sort(function(a, b) {
                    return max[a] - max[b];
                }),
                top = 0,
                bottom = 0,
    ...
    function d4_dsv(delimiter, mimeType) {
        ...
        function dsv(url, callback) {
            return d4.xhr(url, mimeType, callback).response(response);
        }
    ...
    d4.geo.stream = function(object, listener) {
        if (d4_geo_streamObjectType.hasOwnProperty(object.type)) {
    ...
    function d4_geo_streamGeometry(geometry, listener) {
        if (d4_geo_streamGeometryType.hasOwnProperty(geometry.type)) {
    ...
    d4.geo.albersUsa = function() {
        ...
        var alaska = d4.geo.albers().rotate([160, 0]).center([0, 60]).parallels([55, 65]);
        var hawaii = d4.geo.albers().rotate([160, 0]).center([0, 20]).parallels([8, 18]);
        var puertoRico = d4.geo.albers().rotate([60, 0]).center([0, 10]).parallels([8, 18]);
    ...
    (d4.geo.albers = function() {
        var φ0 = 29.5 * d4_radians,
            φ1 = 45.5 * d4_radians,
            m = d4_geo_projectionMutator(d4_geo_albers),
            p = m(φ0, φ1);
        p.parallels = function(_) {
            if (!arguments.length) return [φ0 * d4_degrees, φ1 * d4_degrees];
            return m(φ0 = _[0] * d4_radians, φ1 = _[1] * d4_radians);
        };
    ...
    d4.geo.bounds = d4_geo_bounds(d4_identity);

    function d4_geo_bounds(projectStream) {
        var x0, y0, x1, y1;
        var bound = {
            point: boundPoint,
            lineStart: d4_noop,
        ...
        function boundPolygonLineEnd() {
            bound.point = bound.lineEnd = d4_noop;
        }
        return function(feature) {
            y1 = x1 = -(x0 = y0 = Infinity);
            d4.geo.stream(feature, projectStream(bound));
            return [
                [x0, y0],
                [x1, y1]
            ];
    ...
    function d4_geo_circleInterpolate(radians, precision) {
        ...
        return function(listener) {
            ...
            var clip = {
                ...
                polygonEnd: function() {
                    ...
                    if (segments.length) {
                        d4_geo_clipPolygon(segments, interpolate, listener);
    ...
    function d4_geo_clipPolygon(segments, interpolate, listener) {
        var subject = [],
            clip = [];
        ...
        clip.sort(d4_geo_clipSort);
        d4_geo_clipLinkCircular(subject);
        d4_geo_clipLinkCircular(clip);
    ...
    function d4_geo_clipLinkCircular(array) {
    ...
    function d4_geo_clipCircle(degrees) {
        var radians = degrees * d4_radians,
            cr = Math.cos(radians),
            interpolate = d4_geo_circleInterpolate(radians, 6 * d4_radians);
        ...
        function clipLine(listener) {
            ...
            return {
                ...
                point: function(λ, φ) {
                    ...
                    if (v && (!point0 || !d4_geo_sphericalEqual(point0, point1))) listener.point(point1[0], point1[1]);
                    point0 = point1;
                },
                lineEnd: function() {
        ...
        function intersect(a, b) {
            var pa = d4_geo_cartesian(a, 0),
                pb = d4_geo_cartesian(b, 0);
            var n1 = [1, 0, 0],
            ...
            var u = n1xn2,
                ...
                t = Math.sqrt(w * w - uu * (d4_geo_cartesianDot(A, A) - 1)),
                q = d4_geo_cartesianScale(u, (-w - t) / uu);
            d4_geo_cartesianAdd(q, A);
            return d4_geo_spherical(q);
        }
    }
    ...
    (d4.geo.equirectangular = function() {
        return d4_geo_projection(d4_geo_equirectangular).scale(250 / π);
    ...
    d4.geo.graticule = function() {
        ...
        function lines() {
            return d4.range(Math.ceil(x0 / dx) * dx, x1, dx).map(x).concat(d4.range(Math.ceil(y0 / dy) * dy, y1, dy).map(y));
        }
        ...
        graticule.precision = function(_) {
            if (!arguments.length) return precision;
            precision = +_;
            x = d4_geo_graticuleX(y0, y1, precision);
    ...
    function d4_geo_interpolate(x0, y0, x1, y1) {
        var cy0 = Math.cos(y0),
            ...
            kx1 = cy1 * Math.cos(x1),
            ky1 = cy1 * Math.sin(x1),
            d = 2 * Math.asin(Math.sqrt(d4_geo_haversin(y1 - y0) + cy0 * cy1 * d4_geo_haversin(x1 - x0))),
            k = 1 / Math.sin(d);
    ...
    d4.geo.greatArc = function() {
        var source = d4_source,
            source_, target = d4_target,
            target_, precision = 6 * d4_radians,
            interpolate;

        function greatArc() {
            var p0 = source_ || source.apply(this, arguments),
                p1 = target_ || target.apply(this, arguments),
                i = interpolate || d4.geo.interpolate(p0, p1),
                t = 0,
        ...
        greatArc.distance = function() {
            return (interpolate || d4.geo.interpolate(source_ || source.apply(this, arguments), target_ || target.apply(this, arguments))).distance;
        };
        greatArc.source = function(_) {
            if (!arguments.length) return source;
            source = _, source_ = typeof _ === "function" ? null : _;
            interpolate = source_ && target_ ? d4.geo.interpolate(source_, target_) : null;
            return greatArc;
        };
        ...
        greatArc.precision = function(_) {
            if (!arguments.length) return precision / d4_radians;
            precision = _ * d4_radians;
            return greatArc;
        };
        return greatArc;
    };
    ...
    (d4.geo.mercator = function() {
        return d4_geo_projection(d4_geo_mercator).scale(500);
    }).raw = d4_geo_mercator;
    ...
    d4.geo.path = function() {
        ...
        path.bounds = function(object) {
            return d4_geo_bounds(projectStream)(object);
        };
    ...
    function d4_geo_pathCircle(radius) {
    ...
    function d4_geo_pathBuffer() {
        var pointCircle = d4_geo_pathCircle(4.5),
            buffer = [];
        var stream = {
            point: point,
            lineStart: function() {
            ...
            pointRadius: function(_) {
                pointCircle = d4_geo_pathCircle(_);
                return stream;
            },
            result: function() {
                if (buffer.length) {
    ...
    var d4_geo_areaSum, d4_geo_areaRingU, d4_geo_areaRingV;
    var d4_geo_area = {
        sphere: function() {
            d4_geo_areaSum += 4 * π;
        },
        ...
        lineEnd: d4_noop,
        polygonStart: function() {
            d4_geo_areaRingU = 1, d4_geo_areaRingV = 0;
            d4_geo_area.lineStart = d4_geo_areaRingStart;
        },
        polygonEnd: function() {
            var area = 2 * Math.atan2(d4_geo_areaRingV, d4_geo_areaRingU);
    ...
    function d4_geo_areaRingStart() {
        ...
        function nextPoint(λ, φ) {
            ...
            var dλ = λ - λ0,
                ...
                v = k * Math.sin(dλ);
            d4_geo_areaRingU = u0 * u - v0 * v;
            d4_geo_areaRingV = v0 * u + u0 * v;
    ...
    function d4_geo_projectionMutator(projectAt) {
        var project, rotate, projectRotate, projectResample = d4_geo_resample(function(x, y) {
            ...
            δφ = 0,
            δγ = 0,
            δx, δy, clip = d4_geo_clipAntimeridian,
            clipAngle = null;
        ...
        projection.stream = function(stream) {
            return d4_geo_projectionRadiansRotate(rotate, clip(projectResample(stream)));
        };
        projection.clipAngle = function(_) {
            if (!arguments.length) return clipAngle;
            clip = _ == null ? (clipAngle = _, d4_geo_clipAntimeridian) : d4_geo_clipCircle(clipAngle = +_);
            return projection;
        };
    ...
    d4.geom.voronoi = function(vertices) {
        var polygons = vertices.map(function() {
            ...
            Z = 1e6;
        d4_voronoi_tessellate(vertices, function(e) {
    ...
    var d4_voronoi_opposite = {
        l: "r",
        r: "l"
    };

    function d4_voronoi_tessellate(vertices, callback) {
        var Sites = {
        ...
        var EdgeList = {
            ...
            rightRegion: function(he) {
                return he.edge == null ? Sites.bottomSite : he.edge.region[d4_voronoi_opposite[he.side]];
            }
        };
        var Geom = {
            ...
            endPoint: function(edge, side, site) {
                edge.ep[side] = site;
                if (!edge.ep[d4_voronoi_opposite[side]]) return;
                callback(edge);
            },
        ...
        while (true) {
            ...
            if (newSite && (EventQueue.empty() || newSite.y < newIntStar.y || newSite.y == newIntStar.y && newSite.x < newIntStar.x)) {
                ...
                Geom.endPoint(e, d4_voronoi_opposite[pm], v);
    ...
    d4.geom.quadtree = function(points, x1, y1, x2, y2) {
        ...
        root.visit = function(f) {
            d4_geom_quadtreeVisit(f, root, x1, y1, x2, y2);
        };
    ...
    var d4_time_formatDateTime = "%a %b %e %X %Y",
        d4_time_formatDate = "%m/%d/%Y",
        d4_time_formatTime = "%H:%M:%S";
    var d4_time_days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
        d4_time_dayAbbreviations = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
        d4_time_months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
        d4_time_monthAbbreviations = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    ...
    function d4_time_scale(linear, methods, format) {
        ...
        scale.ticks = function(m, k) {
            var extent = d4_time_scaleExtent(scale.domain());
    ...
    function d4_time_scaleExtent(domain) {
        var start = domain[0],
            stop = domain[domain.length - 1];
        return start < stop ? [start, stop] : [stop, start];
    }
    ...
    var d4_time_scaleUTCFormats = [
        ...
        [d4.time.format.utc("%B"), function(d) {
